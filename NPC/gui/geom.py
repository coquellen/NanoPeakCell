...
#def reconstruct_MPCCD(data,geom_params):
#    reconstructed = np.zeros((1600,1600))
#    return reconstructed
...
def reconstruct(data,geom):
    ...
    if data.shape == shape:
        if len(geom_params) == 64: return reconstruct(data,geom_params,size=1800)
        if len(geom_params) == 8: return reconstruct(data, geom_params, size=1600)
